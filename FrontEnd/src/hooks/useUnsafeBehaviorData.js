...
export function useUnsafeBehaviorData(filters = {}, enabled = true) {
  ...
  useEffect(() => {
    ...
    const fetchMetrics = async (showLoader) => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/unsafe-behavior/metrics${queryString}`, {
          cache: "no-store",
        });

        if (!response.ok) {
          throw new Error(`Unsafe Behavior metrics request failed with status ${response.status}.`);
        }
        ...
        if (isDisposed) return;
        setError(fetchError instanceof Error ? fetchError.message : "Unable to load Unsafe Behavior metrics.");
      } finally {
        if (!isDisposed) {
          setLoading(false);
        }
