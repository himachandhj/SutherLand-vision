...
PPE_MODEL_SOURCES = ["keremberke/yolov8m-hard-hat-detection", "ppe.pt"]
...
def open_video(path):
    if not os.path.isfile(path): sys.exit(f"Input not found: {path}")
...
def create_writer(path, fps, w, h):
    ...
    for cc in ("avc1", "h264", "H264", "mp4v"):
        ...
        wr.release()
    sys.exit("No MP4 codec available.")
...
def parse_args():
    ...
    p.add_argument("--model",        default="yolo11m.pt",
                   help="YOLO person tracking model (default: yolo11m.pt)")
...
def main():
    ...
    out_p   = build_out(input_p, args.output)

    try:
        person_model = YOLO(args.model)
    except Exception as e:
        sys.exit(f"Failed to load person model '{args.model}': {e}")

    ppe_model, ppe_names = load_ppe_model(args.ppe_model)
    detector = PPEDetector(ppe_model, ppe_names, args.ppe_conf, device)
        ...
        while cap.isOpened():
            ...
            try:
                res = person_model.track(
                    source=frame, classes=[0], conf=args.conf,
            ...
            for i, (bbox, vis) in enumerate(zip(boxes, vis_list)):
                ...
                if tid not in workers:
                    workers[tid] = WorkerState(tid, args.smoothing)
                workers[tid].update(frame_num, ppe_list[i])
                draw_person(frame, bbox, workers[tid], args.line_width)
    ...
    if not os.path.isfile(out_p) or os.path.getsize(out_p) == 0:
        sys.exit(f"Output missing or empty: {out_p}")
